...
```
auto f = std::bind(add, std::ref(x), 5);
```
...
**现代 C++（C++14+）：能用 lambda 就别用 bind**

------

六、bind 现在主要用在哪？

虽然 lambda 更好，但 `bind` 仍有它的“历史地位”和少数合理用途：
...
asio::async_read(
    socket,
    buffer,
    std::bind(&Session::onRead, this, _1, _2)
);
```
